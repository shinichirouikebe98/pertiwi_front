          ...
          <form @submit.prevent="updateBerita">
            ...
            <div class="form-layout">
              ...
              <div class="berita">
                    ...
                    <div v-if="validation.news" class="mt-2">
                      <b-alert show variant="danger fixed">{{ validation.news[0] }}</b-alert>
                    </div>
              </div>
              <div class="gambar">
...
export default {
    ...
    mounted(){
      ...
      this.$axios.get(`/api/admin/beritas/${this.$route.params.id}`)
        ...
        .then(response => {
          ...
          this.data.berita = response.data.data.news

          //assing response data to state "data.img"
          this.data.gambar = response.data.data.img
        })
    },
    methods:{
       handleFileChange(e) {
        ...
        //get image
        let image = this.data.gambar = e.target.files[0]
      ...
      async updateBerita(){
           ...
           dataBerita.append('img', this.data.gambar);
           dataBerita.append("_method", "PATCH");

           await this.$axios.post(`api/admin/beritas/${this.$route.params.id}`, dataBerita)
           .then(() => {
              this.$router.push({
                name: 'admin-news'
              })

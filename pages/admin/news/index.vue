<template>
  <div class="container">
    <h1 style="letter-spacing:4px; margin-top:40px;">NEWS LIST</h1>
    <b-button :to="{name:'admin-news-create'}" variant="outline-primary" style="margin-left:5px;">+ TAMBAH BERITA</b-button>
...
async asyncData({ $axios, query }) {
      ...
      //fetching news
      const news = await $axios.$get(`/api/admin/beritas?q=${search}&page=${page}`)
      return {
        'news': news.data.data,
  ...
  methods:{
    ...
    deleteBerita(id){
        ...
        this.$axios.delete(`/api/admin/beritas/${id}`)
        .then(() => {
          //feresh data
          this.$nuxt.refresh()
        })      
